...
function mapRequestPath(requestPath, basePath) {
  ...
  else if (relative.startsWith(`${base}/`))
...
async function resolveFile(root, relative) {
  ...
  const resolveExisting = async (value) => {
    ...
    if (root.includes(`.asar${path.sep}`)) {
      try {
        return (await fsp.stat(value)).isFile() ? value : null;
      } catch {
        return null;
      }
    }
...
async function createArchiveServer({
  root,
  basePath = '/hay-day-wiki-archive/',
} = {}) {
  const resolvedRoot = path.resolve(
    root || path.join(__dirname, '..', 'dist', 'pages'),
  );
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (!cookies || cookies.get(cookieName) !== token) {
    ...
    let relative;
    try {
      relative = mapRequestPath(url.pathname, basePath);
    } catch {
      relative = null;
    }
    if (relative === null) {
    ...
    const headers = {
      ...
      'Content-Security-Policy':
        "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://static.wikia.nocookie.net; media-src 'self' https://static.wikia.nocookie.net; font-src 'self' data:; connect-src 'self'; object-src 'none'; frame-src 'none'; base-uri 'none'",
    };
  ...
  return {
    ...
    token,
    port,
    url: `http://127.0.0.1:${port}${prefix}index.html?access=${encodeURIComponent(token)}`,
    close: () =>
