...
export default function CategoriesPage() {
  ...
  return <main className="simple-page reader-page">
    <ArchiveNav />
    <section className="simple-hero"><p className="eyebrow">Archive structure</p><h1>Categories</h1><p>Browse all imported category pages without losing their source revision evidence.</p></section>
    <ArchiveSearch label="Search categories" items={categories.map((item) => ({ title: item.title.replace(/^Category:/, ''), route: item.route, detail: `Source revision ${item.revisionId}` }))} />
  </main>;
}
