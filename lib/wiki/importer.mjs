...
export class WikiImporter {
  ...
  async run({ resume = true, maxPages = Infinity, refreshMedia = false } = {}) {
    ...
    if (refreshMedia) await this.rebuildMediaGraph(working);
  ...
  async rebuildMediaGraph(working) {
    ...
    for (const record of Object.values(working.pages)) {
      ...
      for (const title of titles) mediaTitles.add(title);
    }
    working.media = {};
    const sortedTitles = [...mediaTitles].sort((a, b) => a.localeCompare(b));
    const imageInfo = await this.client.imageInfo(sortedTitles);
    for (const image of imageInfo)
      working.media[mediaKey(image.title)] = this.mediaRecord(image);
...
function normalizeMediaState(media) {
  ...
  for (const record of Object.values(media ?? {})) {
    ...
    const candidate = { ...record, mediaId: key, title: key, route: mediaRoute(key) };
